...
\label{sec:prob}
	...
	\label{sec:meth1}
		...
		\begin{flalign}
			\centering
			E = \frac{1}{n} \sum_{j=1}^{n}[p(x_{j}) - h(x_{j})]^{2} + \omega \cdot \left| \left( \sum_{i=1}^{K} P_{i} \right) - 1 \right| \label{eq:mean-square-error}
		\end{flalign}
		...
		Die Minimierung einer solchen Funktion ist laut \cite{cuevas-meth1} kein leichtes Problem. Hierfür lautet die Begründung wie folgt:\\
